...
const Published = ({ register, errors, required }) => {
  return (
    <Form.Group className={ errors.published ? 'has-error' : null }>
      <Form.Control
        as="textarea"
        rows={4}
        ...
        name="published"
        ref={ register({
          required: required ? 'Необходимо указать публикации проекта' : false
        })} />
      <Form.Text>
